...
| What | Where | Role |
|---|---|---|
| Skill `/adv-psy:adv-psy` | `skills/adv-psy/SKILL.md` | config gate, triage, anonymisation, convening the council, writing to memory |
| Protocols | `skills/adv-psy/protocols/` | triage gate, safety (stop gate, tiers, contacts), verdict format, memory, lens spec |
| Method conflicts | `skills/adv-psy/references/cross-method-patterns.md` | "target → lens" routing and what to do when lenses disagree |
...
- `${CLAUDE_PLUGIN_ROOT}` and `${user_config.PSY_MEMORY_DIR}` are substituted **inside the text of SKILL.md and agents** — there you can write them directly.
