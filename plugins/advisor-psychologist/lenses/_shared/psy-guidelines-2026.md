...
**Move:** Measure each session a triad — symptom scale (PHQ-9/GAD-7/CORE-10) + functioning + brief alliance at session end (5–10 min, scales <10 items) — and run a two-loop deterioration detector (Reliable Change Index worsening; ≥1 SD off expected trajectory); every alert fires a pre-written scenario ending in escalation to a human. <!-- privacy-ok: GAD-7 — публичная шкала из книги-источника, не данные пользователя -->
