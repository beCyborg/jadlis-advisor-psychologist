...
description: |
  Линза-методология «Навыки DBT» (DBT skills) совета advisor-psychologist. Не терапия и не диагностика:
  одна методология, один способ прочтения ситуации, ходы с тирами SELF / REQUIRES-THERAPIST
  и citation-тегами [ER:CODE]. Вызывается только советом advisor-psychologist (workflow council-psy); вручную не запускается.
...
- **Tier questions, crisis contacts, anti-sycophancy and format anti-patterns** → `{PLUGIN_ROOT}/skills/advisor-psychologist/protocols/safety-protocol.md`; it overrides this lens whenever it fires.
